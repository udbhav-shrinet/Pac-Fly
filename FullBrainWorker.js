...
function tick() {
	...
	if (groupStimuli) {
		for (var gs = 0; gs < groupStimuli.length; gs++) {
			var stim = groupStimuli[gs];
			if (!stim) continue;
			var gsStart = groupOffset[gs];
			var gsEnd = groupOffset[gs + 1];
			if (!groupActive[gs]) {
				groupActive[gs] = 1;
				groupCooldown[gs] = COOLDOWN_TICKS;
			}
			for (var gsi = gsStart; gsi < gsEnd; gsi++) {
				if (refractory[gsi] === 0) V[gsi] += stim;
			}
		}
		groupStimuli.fill(0);
	}
...
self.onmessage = function (e) {
	...
	case 'stimulateGroups':
		if (!groupStimuli) groupStimuli = new Float32Array(numGroups);
		for (var sg = 0; sg < e.data.groups.length; sg++) {
			var groupIndex = e.data.groups[sg];
			if (groupIndex >= 0 && groupIndex < numGroups) {
				groupStimuli[groupIndex] += e.data.intensities[sg];
			}
		}
		break;
